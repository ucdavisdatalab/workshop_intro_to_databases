...
software that is designed to interact with them, like in the case of .xlsx files
and Microsoft Excel. However, a flat file can also be as simple as a text file,
like a .csv or .json.
...
system, often abbreviated DBMS. Databases have several key characteristics that differentiate them from other data stores.
...
Relational Database Management Systems (RDBMSs), or relational databases, are one type of database. When people use the word "database", nine
times out of ten they are referring to a relational database. Like
spreadsheets, RDBMSs impose a tabular structure on the data they contain.
Unlike spreadsheets the structure of that data can be fairly complex.
Relational databases are made up of tables that are connected through shared
columns, called indexes or keys.
...
the same set of data. However, it has a higher barrier to entry than a graphical
user interface like Excel.
...
The SQL query I used to extract the Die Hard datais also reproducible. If I want
to collaborate on my analysis of Die Hard a colleague, I don't have to send the
Die Hard data set over to my collaborator. Instead I can provide them access to
to the IMDB database and my SQL query. If I find problems in the data at a later
...
All changes to the data can be made to the singleton values in their original
tables. For example, if you accidentally left out the second "L" in Bruce
Willis's name when entering the data, you would only need to modify his name
once in the "people" table. Then, any time you extract data from the people
table, that change will be automatically propagated to our new data. While it
may be easy to successfully fix 5 errors, it will be much harder to fix 50, or
5,000. This is especially true if they are scattered throughout your data set
and not just in a single column.
...
Amazon AWS), but these are the most common and widely supported.

+-------------+-------------+---------------+-------------+-------------+
| Software    | Ease of Use | D             | Cost        | License     |
|             |             | ocumentation/ |             |             |
|             |             |               |             |             |
|             |             | Support       |             |             |
+=============+=============+===============+=============+=============+
...
|             |             | support       |             |             |
+-------------+-------------+---------------+-------------+-------------+
| PostgreSQL  | Harder      | Spotty        | \$0 + cost  | Open Source |
|             |             | d             | of hosting  |             |
|             |             | ocumentation, |             |             |
|             |             | but with      |             |             |
|             |             | robust        |             |             |
...
|             |             | support       |             |             |
+-------------+-------------+---------------+-------------+-------------+
| MySQL       | Easier      | Large         | \$0 - Low   | Partially   |
|             |             | community     | Cost        | Open Source |
|             |             | base, No      |             |             |
|             |             | professional  |             |             |
|             |             | support       |             |             |
|             |             | without paid  |             |             |
|             |             | subscription  |             |             |
+-------------+-------------+---------------+-------------+-------------+
| Microsoft   | Easier      | Robust        | High Cost   | Proprietary |
| SQL Server  |             | professional  |             |             |
|             |             | and community |             |             |
|             |             | support       |             |             |
+-------------+-------------+---------------+-------------+-------------+
| Oracle      | Harder      | Robust        | High Cost   | Proprietary |
|             |             | professional  |             |             |
|             |             | and some      |             |             |
|             |             | community     |             |             |
|             |             | support       |             |             |
...
with textual data, which can easily take up many terabytes of storage.
...
structures while others specialize in one.

+-----------------------+-----------------------------+-----------------------+
| Software              | Data Structure              | Cost                  |
+=======================+=============================+=======================+
| MongoDB               | Tree/Document               |                       |
+-----------------------+-----------------------------+-----------------------+
| Couchbase             | Tree/Document               |                       |
+-----------------------+-----------------------------+-----------------------+
